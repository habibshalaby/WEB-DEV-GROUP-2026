...
def calculate_total(gym: str, gym_option: str, addons: list, member_type: str) -> dict:
    ...
    # Standalone items (no gym membership selected) – re-check if correct keys used
    discount_amount    = round((base_monthly / (1 - discount_rate) - base_monthly) if discount_rate else 0, 2)
    final_monthly      = round(base_monthly, 2)
    total_first_month  = round(joining_fee + final_monthly, 2)

    return {
        'joining_fee':       joining_fee,
        'base_monthly':      round(base_monthly / (1 - discount_rate) if discount_rate else base_monthly, 2),
...
def checkout():
    ...
    POST → create Member record in DB, generate membership ID,
    ...
    if request.method == 'POST':
        ...
        new_member = Member(
            ...
            total_price   = order['total_first_month'],
        )
    ...
    return render_template('checkout.html', user=user, order=order)
...
def login():
    """
    Allows an existing member to log in using their email + password.
    On success, fetches their membership record and displays it.
